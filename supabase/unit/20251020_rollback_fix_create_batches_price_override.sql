-- Rollback: Restore original create_batches_from_po function
-- Date: 2025-10-20
-- Use this if removing price update logic causes issues
-- This restores the function WITH price update logic (original buggy version)

-- Drop the fixed function
DROP FUNCTION IF EXISTS public.create_batches_from_po(UUID);

-- Recreate ORIGINAL function with price update logic
...
BEGIN
  ...
  LOOP
    ...
    -- ⚠️ WARNING: This logic has a bug with multi-unit products!
    -- It writes base unit prices to products.current_selling_price field
    -- which should contain default unit prices, causing incorrect display
...
'ROLLBACK VERSION: Original function with price update logic.
⚠️ Known bug: Overwrites default unit prices with base unit prices.
Only use this rollback if the fixed version causes critical issues.';
