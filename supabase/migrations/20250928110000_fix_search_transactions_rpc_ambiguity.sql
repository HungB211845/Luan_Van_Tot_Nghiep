'''-- Fixes an ambiguity error in the search_transactions function.
...
-- The fix is to rename the local variable that holds the current user's store ID to 'v_current_store_id'
-- to remove any ambiguity for the PostgreSQL query planner.
...
BEGIN
    ...
    SELECT up.store_id INTO v_current_store_id FROM public.user_profiles up WHERE up.id = auth.uid()::text;
