'''-- Fixes an ambiguity error in the search_transactions function by aliasing the output column.
-- The error occurs because the function's return signature includes a column named 'store_id',
-- which conflicts with the 'store_id' columns in the tables being queried.
-- The fix is to rename the output column to 'transaction_store_id' and alias 't.store_id' to it in the SELECT statement.
-- The internal variable 'current_user_store_id' is kept as is.
...
RETURNS TABLE (
    ...
    transaction_store_id UUID, -- Renamed output column
    customer_id UUID,
...
AS $$
DECLARE
    current_user_store_id UUID; -- Reverted to original variable name
    query_sql TEXT;
    where_clauses TEXT[] := ARRAY[]::TEXT[];
    offset_val INT;
BEGIN
    -- 1. Security: Get the store_id of the currently authenticated user
    SELECT up.store_id INTO current_user_store_id FROM public.user_profiles up WHERE up.id = auth.uid()::text;
    ...
    where_clauses := array_append(where_clauses, 't.store_id = ' || quote_literal(current_user_store_id));
    ...
    IF p_payment_methods IS NOT NULL AND array_length(p_payment_methods, 1) > 0 THEN
        where_clauses := array_append(where_clauses, 't.payment_method = ANY(' || quote_literal(p_payment_methods) || ')');
    END IF;

    IF p_customer_ids IS NOT NULL AND array_length(p_customer_ids, 1) > 0 THEN
        where_clauses := array_append(where_clauses, 't.customer_id = ANY(' || quote_literal(p_customer_ids) || ')');
    END IF;
    ...
    query_sql := '
        SELECT
            t.id, t.created_at, t.store_id AS transaction_store_id, t.customer_id, t.total_amount,
