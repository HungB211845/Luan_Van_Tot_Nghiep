  ...
  Future<void> addToCart(Product product, int quantity, {double? customPrice, ProductUnit? selectedUnit}) async {
    ...
    if (unit == null) {
      ...
      if (unit == null) {
        ...
        unit = ProductUnit(
          id: '', // Keep empty string for temporary units - will be handled in checkout
          productId: product.id,
  ...
  Future<String?> checkout({
    ...
    try {
      ...
      final transactionItems = _cartItems.map((cartItem) {
        // 🔥 FIX: Convert empty string UUID to null to avoid Postgres error
        final unitId = cartItem.selectedUnitId?.isEmpty == true ? null : cartItem.selectedUnitId;
        ...
        final txItem = TransactionItem(
          ...
          createdAt: DateTime.now(),
          storeId: BaseService.getDefaultStoreId(),
          unitId: unitId, // ← FIX: Use null if empty string to avoid UUID error
